...
func TestEndToEndIndexerIntegration(t *testing.T) {
	...
	conn, err := net.DialTimeout("tcp", "localhost:4000", 100*time.Millisecond)
	if err != nil {
		t.Skip("Skipping live integration test: local LiteLLM server on localhost:4000 is unreachable")
	}
	conn.Close()
	...
	added, modified, deleted, err := merkle.UpdateIndex(tmpCodebase)
	if err != nil {
	...
	// Verify that Codebase Profile was generated and saved as personal category memory
	mockQueryVector, err := llm.GetEmbedding("codebase overview profile")
	if err != nil {
		t.Fatalf("failed to get real embedding: %v", err)
	}

	results, err := db.SearchMemories(mockQueryVector, "personal", tmpCodebase, 5)
	if err != nil {
	...
	for _, res := range results {
		if strings.Contains(res.Content, "[Codebase Profile] Codebase:") {
			hasProfile = true
			break
		}
	}
	if !hasProfile {
		t.Error("codebase profile was not successfully registered in global memory store")
	}
...
func TestMultiCodebaseAndPersonalMemoriesIntegration(t *testing.T) {
	...
	_, _, _, err = merkle.UpdateIndex(tmpCodebaseA)
	if err != nil {
		t.Fatalf("failed to index codebase A: %v", err)
	}

	_, _, _, err = merkle.UpdateIndex(tmpCodebaseB)
	if err != nil {
	...
	err = db.SaveMemory("p1", "User prefers functional programming in Go", "personal", "", mockEmbed1)
	if err != nil {
	...
	err = db.SaveMemory("p2", "User prefers space indentation of size 2", "personal", "", mockEmbed2)
	if err != nil {
	...
	results, err := db.SearchMemories(mockEmbed1, "personal", tmpCodebaseA, 5)
	if err != nil {
	...
	resultsA, err := db.SearchMemories(mockEmbed1, "project", tmpCodebaseA, 5)
	if err != nil {
